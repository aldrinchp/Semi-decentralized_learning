...
class Server:
    ...
    async def _process_polling(self, msg, websocket):
        ...
        if self.pending_rotation_msg is not None:
            ...
            # FIX: Also track by IP to handle agents that restarted with new IDs
            agent_ip_from_msg = None
            for a in self.sm.agent_set:
                if a['agent_id'] == agent_id:
                    agent_ip_from_msg = a['agent_ip']
                    break
            ...
            if agent_id not in self.rotation_notified_agents:
                ...
                logging.info(f'🔄 Rotation message sent to {agent_id} (IP: {agent_ip_from_msg}) via polling')
            ...
            logging.info(f'📊 Rotation status: notified={len(self.rotation_notified_agents)}/{len(all_agent_ids)} agents')
            logging.info(f'    Notified IDs: {self.rotation_notified_agents}')
            logging.info(f'    All agent IDs: {all_agent_ids}')
            
            # Check: Have enough agents been notified?
            # FIX: Use count-based check instead of ID-based to handle ID mismatches
            # We consider rotation complete when we've notified at least as many unique agents
            # as are currently registered (even if IDs don't match exactly)
            if len(self.rotation_notified_agents) >= len(all_agent_ids):
                logging.info(f'✅ All {len(all_agent_ids)} agents notified of rotation (notified {len(self.rotation_notified_agents)}).')
                ...
                # Check if THIS aggregator is the winner (compare by IP for consistency with agents)
                # FIX: Use IP comparison instead of ID comparison to avoid inconsistencies
                i_am_winner = (self.aggr_ip == winner_ip)
                logging.info(f'DEBUG: My IP is {self.aggr_ip}, winner IP is {winner_ip}, I am winner? {i_am_winner}')
                
                if i_am_winner:
                    logging.info(f'This aggregator won the rotation. Staying as aggregator.')
                    # Re-register ourselves in DB to ensure we're the current aggregator
                    await self._register_aggregator_in_db()
                    ...
                    # CRITICAL: Clear ourselves from DB BEFORE exiting so new aggregator can register
                    try:
                        ...
                        logging.info(f'🧹 Cleared aggregator from DB before exit: {resp}')
                    except Exception as e:
                        logging.error(f'Failed to clear aggregator from DB: {e}')
                    ...
                    # Small delay to ensure DB is updated before exit
                    await asyncio.sleep(2)  # Increased delay for port release
                    logging.info(f'🔄 Exiting aggregator process for role transition...')
                    os._exit(0)
            else:
                remaining_count = len(all_agent_ids) - len(self.rotation_notified_agents)
                logging.info(f'Rotation sent to {agent_id}. Waiting for ~{remaining_count} more agents to poll...')
            return
    ...
    async def model_synthesis_routine(self):
        ...
        :return:
        """
        # Counter for periodic "am I still aggregator" check (every 6 cycles = ~30s)
        aggregator_check_counter = 0
        AGGREGATOR_CHECK_INTERVAL = 6
        
        while True:
            # Periodic check (frequency is specified in the JSON config file)
            await asyncio.sleep(self.round_interval)
            
            # Periodically verify we're still the aggregator in DB
            aggregator_check_counter += 1
            if aggregator_check_counter >= AGGREGATOR_CHECK_INTERVAL:
                aggregator_check_counter = 0
                should_continue = await self._verify_still_aggregator()
                if not should_continue:
                    logging.warning(f"🛑 No longer the aggregator - shutting down")
                    os._exit(0)
    ...
    async def _verify_still_aggregator(self) -> bool:
        """
        Verify that this node is still the registered aggregator in the DB.
        Returns True if still aggregator, False if another node took over.
        """
        try:
            from fl_main.lib.util.states import DBMsgType
            ...
            if resp and resp[0] == 'aggregator':
                ...
                # Compare DB aggregator IP with our IP
                if db_agg_ip != self.aggr_ip:
                    logging.warning(f"⚠️ Another aggregator ({db_agg_ip}) registered in DB - I ({self.aggr_ip}) should step down")
                    return False
                return True
            elif resp and resp[0] == 'none':
                # No aggregator in DB - I should re-register or step down
                logging.warning(f"⚠️ No aggregator registered in DB - re-registering myself")
                # Try to re-register ourselves
                await self._register_aggregator_in_db()
                return True
            else:
                # Couldn't contact DB - keep running (fail safe)
                logging.debug(f"Could not verify aggregator status with DB: {resp}")
                return True
        except Exception as e:
            logging.debug(f"Error verifying aggregator status (non-critical): {e}")
            return True  # Keep running if we can't check

    async def _register_aggregator_in_db(self):
        """
        Register/re-register this aggregator in the DB.
        """
        try:
            from fl_main.lib.util.states import DBMsgType
            msg = [DBMsgType.update_aggregator.value, self.sm.id, self.aggr_ip, int(self.reg_socket)]
            resp = await send(msg, self.db_ip, self.db_socket)
            
            if resp and resp[0] == 'updated':
                logging.info(f"✅ Re-registered as aggregator in DB: {self.aggr_ip}:{self.reg_socket}")
            else:
                logging.warning(f"⚠️ Failed to re-register as aggregator: {resp}")
        except Exception as e:
            logging.error(f"Error re-registering as aggregator: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Register ourselves as the aggregator in DB immediately on startup
        # This is crucial for agents to find us after a rotation
        async def register_on_startup():
            try:
                from fl_main.lib.util.states import DBMsgType
                msg = [DBMsgType.update_aggregator.value, s.sm.id, s.aggr_ip, int(s.reg_socket)]
                resp = await send(msg, s.db_ip, s.db_socket)
                if resp and resp[0] == 'updated':
                    logging.info(f"✅ Registered as aggregator in DB: {s.aggr_ip}:{s.reg_socket}")
                else:
                    logging.warning(f"⚠️ Failed to register as aggregator in DB: {resp}")
            except Exception as e:
                logging.error(f"Error registering aggregator in DB on startup: {e}")
        
        # Run registration before starting servers
        asyncio.get_event_loop().run_until_complete(register_on_startup())
