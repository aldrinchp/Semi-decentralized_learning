...
class Client:
    ...
    async def participate(self):
        ...
        # Wait for other agents to register and for any existing aggregator to start
        await asyncio.sleep(10)  # Give time for aggregator to fully start
        ...
        # Step 2.5: Check if discovered aggregator is MYSELF (from a previous election I won)
        if agg_ip:
            device_ip = self.config.get('device_ip', self.agent_ip)
            if device_ip == 'CHANGE_ME':
                device_ip = self.agent_ip
            if agg_ip == device_ip:
                logging.info(f'🏆 Discovered that I ({device_ip}) am the registered aggregator - starting server!')
        ...
        if not agg_ip:
            ...
            scores = {my_id: my_score}  # Send my score for election
        ...
        if resp is None:
            ...
            # Re-register with a new score
            my_id, my_score = await self._register_in_db()
            
            # GRACE PERIOD: Wait for other nodes to also detect the failure and re-register
            # This prevents the first node to detect failure from always winning
            grace_period = 15  # seconds - should be longer than polling interval (5s) * 2
            logging.info(f'⏳ Waiting {grace_period}s grace period for other nodes to re-register...')
            await asyncio.sleep(grace_period)
            
            # Check if another node already triggered election while we waited
            existing_agg_ip, existing_agg_socket = await self._discover_aggregator_from_db(verify_alive=False)
            if existing_agg_ip and existing_agg_ip != self.aggr_ip:
                # Someone else already elected a new aggregator
                logging.info(f'📡 New aggregator already elected during grace period: {existing_agg_ip}:{existing_agg_socket}')
                new_agg_ip, new_agg_socket = existing_agg_ip, existing_agg_socket
            else:
                # Trigger election with my score
    ...
    async def model_exchange_routine(self):
        ...
        :return:
        """
        # Counter for periodic aggregator check (every 6 cycles = ~30 seconds)
        ...
        while True:
            ...
            # Periodically check if aggregator in DB has changed
            aggregator_check_counter += 1
            if aggregator_check_counter >= AGGREGATOR_CHECK_INTERVAL:
                aggregator_check_counter = 0
                await self._check_aggregator_changed()
    ...
    async def wait_models(self, websocket, path):
        ...
        if msg_type == AggMsgType.rotation:
            ...
            if i_am_winner:
                ...
                # FIX: Agent lost rotation - must exit to reconnect to new aggregator
                logging.info(f'📡 Agent lost rotation. Exiting to re-register with new aggregator at {winner_ip}')
                os._exit(0)
            return
    ...
    def send_recall_metric(self, recall_value):
        ...
        try:
            ...
            # Use asyncio to properly await the async send() function
            try:
                loop = asyncio.get_running_loop()
                # If there's already a running loop, create a task
                future = asyncio.ensure_future(send(recall_msg, aggr_ip, int(reg_socket)))
                # Can't await here in sync context, but task will run
                resp = None
            except RuntimeError:
                # No running loop, use asyncio.run()
                resp = asyncio.run(send(recall_msg, aggr_ip, int(reg_socket)))
    ...
    def _promote_to_aggregator(self):
        ...
        The role_supervisor will detect the config change and restart as aggregator.
        ...
        try:
            ...
            # NOTE: Do NOT start server here with subprocess.Popen!
            # The role_supervisor will detect the config change and restart as aggregator.
            # Starting it here would cause "address already in use" errors.
            logging.info(f'🔄 Config updated. role_supervisor will restart as aggregator...')
    ...
    async def _check_aggregator_changed(self):
        """
        Periodically check if the aggregator in DB has changed.
        If it changed, restart to connect to the new aggregator.
        This ensures agents discover new aggregators elected by other nodes.
        """
        from fl_main.lib.util.states import DBMsgType
        ...
        try:
            ...
            if resp and resp[0] == 'aggregator':
                ...
                # Check if aggregator in DB differs from our current one
                if db_agg_ip != self.aggr_ip:
                    # Check if the new aggregator is ME
                    device_ip = self.config.get('device_ip', self.agent_ip)
                    if device_ip == 'CHANGE_ME':
                        device_ip = self.agent_ip
                    
                    if db_agg_ip == device_ip:
                        logging.info(f'🏆 I am the new aggregator in DB! Promoting to aggregator...')
                        self._promote_to_aggregator()
                        os._exit(0)  # Exit to restart as aggregator
                    else:
                        logging.info(f'🔄 Aggregator changed in DB: {self.aggr_ip} → {db_agg_ip}')
                        logging.info(f'🔄 Restarting to connect to new aggregator...')
                        
                        # Update config to point to new aggregator
                        try:
                            cfg_agent_file = set_config_file('agent')
                            cfg_agent = read_config(cfg_agent_file)
                            cfg_agent['role'] = 'agent'
                            cfg_agent['aggr_ip'] = db_agg_ip
                            write_config(cfg_agent_file, cfg_agent)
                        except Exception as e:
                            logging.error(f'Failed to update config: {e}')
                        
                        os._exit(0)  # Exit to restart with new aggregator
        except Exception as e:
            logging.debug(f'Aggregator check failed (non-critical): {e}')
