...
class SQLiteDBHandler:
    ...
    def upsert_agent(self, agent_id: str, ip: str, socket: int, score: int = 0):
        """
        ...
        try:
            ...
            if row is not None:
                ...
                if existing_id != agent_id:
                    ...
                    # Same agent_id, just update score and last_seen
                    c.execute('''
                        UPDATE agents SET last_seen = ?, score = ?
                        WHERE agent_id = ?;
                    ''', (now, score, agent_id))
                    conn.commit()
                    logging.info(f"upsert_agent: updated score for agent_id={agent_id} -> score={score}")
                    return True
    ...
    def get_all_agents_with_scores(self):
        """
        Devuelve una lista de (agent_id, ip, socket, score) para todos los agentes
        registrados en la tabla agents.
        """
        conn = sqlite3.connect(self.db_file)
        c = conn.cursor()
        ...
        try:
            ...
            logging.error(f"Error leyendo agentes con scores desde DB: {e}")
            rows = []
        finally:
            conn.close()
