...
class PseudoDB:
    ...
    async def handler(self, websocket, path):
        ...
        if msg_type == DBMsgType.push.value:  # models
            ...
            self.dbhandler.upsert_agent(agent_id, ip, socket, score)
            ...
            # NOTE: scores from message are now ignored - we use DB scores for fairness
            logging.info(f'--- Aggregator election request ---')
            
            # Get ALL registered agents with their scores from DB
            all_agents_with_scores = self.dbhandler.get_all_agents_with_scores()
            logging.info(f'   Registered agents in DB: {len(all_agents_with_scores)}')
            
            if all_agents_with_scores:
                # Build scores dict from DB data: {agent_id: score}
                db_scores = {agent[0]: agent[3] for agent in all_agents_with_scores}
                # Also build IP/socket lookup
                agent_info = {agent[0]: (agent[1], agent[2]) for agent in all_agents_with_scores}
                
                # Log all candidates
                for aid, score in db_scores.items():
                    ip, sock = agent_info[aid]
                    logging.info(f'     Candidate: {ip}:{sock} (score: {score})')
                
                # Find winner (highest score, tie-break by agent_id)
                winner_id = max(db_scores.items(), key=lambda x: (x[1], x[0]))[0]
                winner_ip, winner_socket = agent_info[winner_id]
                winner_score = db_scores[winner_id]
                
                logging.info(f'   Winner: {winner_ip}:{winner_socket} (score: {winner_score})')
                
                self.dbhandler.update_current_aggregator(winner_id, winner_ip, winner_socket)
                reply = ['elected', winner_id, winner_ip, winner_socket, winner_score]
                logging.info(f'   Aggregator elected: {winner_ip}:{winner_socket}')
            else:
                reply = ['election_failed', 'no_agents_registered']
                logging.warning(f'   Election failed: no agents registered in DB')
